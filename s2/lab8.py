...
from utils import get_centrala_url, get_poligon_key, create_image_request, create_payload

robot_description = requests.get(get_centrala_url()+ "/data/" + get_poligon_key() + "/robotid.json").text

response = create_image_request(create_image_upon_description_prompt(robot_description))
